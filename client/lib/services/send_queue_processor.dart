...
/// Надёжная очередь готовых ЗАШИФРОВАННЫХ конвертов (текст, реакции, пины,
/// правки, звонки-как-сообщения, лёгкие события профиля, а также конверт
/// сообщения с файлом — уже после того, как файл залит). Это и есть
/// «лёгкий» поток отправки (ТЗ пользователя). Элемент кладётся в [enqueue]
/// и удаляется только по реальному ack от сервера (см. SendAckRegistry/
/// websocket.go:ackSender). Раньше здесь же жил [runHeavy] — сериализатор
/// самой загрузки файла; теперь загрузка файлов — отдельный поток
/// (PendingSendRetrier), сюда попадает только готовый конверт.
