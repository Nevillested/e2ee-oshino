...
/// перелёт примерно на 10% от пройденного расстояния: при отдыхе на
/// kAttachmentSheetRestFraction = 0.75 экрана кривая сначала долетает
/// примерно до ~0.825 и только потом оседает на 0.75 — всё ещё заметно
/// меньше 1.0 (весь экран), так что клиппинга не будет. Раньше пробовали
/// 8.5 (перелёт почти 100% — при высоте панели около половины экрана это
/// ПРЕВЫШАЛО полную высоту экрана, Flutter обрезал реальный размер по
/// границе, и получалось не "перелёт", а "упёрлось в потолок и рухнуло
/// обратно") и 3.4 (перелёт ~30% — по ощущениям оказалось многовато).
...
/// Было 0.5 — ТЗ пользователя: по умолчанию 3/4 экрана, а не половина.
const double kAttachmentSheetRestFraction = 0.75;
