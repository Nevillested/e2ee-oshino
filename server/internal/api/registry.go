...
//
// Если для этого deviceID уже было соединение — это либо зомби (сокет
// технически ещё не закрыт, но реально не пропускает данные — сервер
// узнал бы об этом только когда его собственный Read() когда-нибудь сам
// отвалится, а это может не произойти очень долго), либо, теоретически,
// честный дубль. В обоих случаях новое соединение — то, с которым
// реально сейчас работает клиент, поэтому старое закрываем сами, не
// дожидаясь, пока оно само когда-нибудь истечёт по таймауту — иначе всё
...
func (reg *ConnectionRegistry) Add(deviceID string, conn *websocket.Conn) {
	...
	//старое соединение (если было) под тем же device_id — закрывать будем
	//уже ПОСЛЕ разблокировки, сетевой I/O под мьютексом всего реестра не
	//делаем (см. тот же принцип в Broadcast ниже).
